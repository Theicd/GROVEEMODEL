...
describe("sensorBlock", () => {
  ...
  it("stage-2 conclusions avoid person claims when absent", () => {
    const world = new WorldMemory();
    world.personPresent = false;
    world.lastMotionLevel = 0.3;
    const lines = inferSensorConclusions(world);
    expect(lines.join(" ")).toMatch(/no person confirmed/i);
    expect(buildSensorBlock(world)).toMatch(/Conclusions:/);
  });
});
