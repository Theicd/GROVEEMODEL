...
export class CameraVisionLoop {
  ...
  start(): void {
    ...
    void preloadPoseDetector().then((ok) => {
      if (ok) this.callbacks.onCameraStatus?.("👁 MoveNet · מוכן");
    }).catch((e) => console.warn("[GROVEE] MoveNet preload", e));
    this.timer = setInterval(() => void this.tick(), CAMERA_LOOP_CONFIG.pollIntervalMs);
  ...
  private async tick(): Promise<void> {
    ...
    await this.runSituationPass(light, light.debouncedPeople.length > 0);
  ...
  private pickDeepVisionReason(lightResult: WorldUpdateResult): string | null {
    ...
    if (since < CAMERA_LOOP_CONFIG.deepVisionMinIntervalMs) return null;
    if (this.lastFrameScore < CAMERA_LOOP_CONFIG.minFrameStabilityForDeep && since < CAMERA_LOOP_CONFIG.deepVisionMaxIntervalMs) {
      return null;
    }
    ...
    if (since >= CAMERA_LOOP_CONFIG.deepVisionMaxIntervalMs) return "heartbeat";
    return null;
  }
  ...
  private async runDeepVision(reason: string): Promise<void> {
    ...
    try {
      const bytes = await captureVideoFrame(this.video, CAMERA_LOOP_CONFIG.analysisMaxWidth);
  ...
  private maybeCharacterTick(): void {
    ...
    if (!this.world.baselineEstablished || !this.world.hasData()) return;
