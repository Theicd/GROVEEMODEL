...
export type PersistedAssistantOptions = {

  /** Document/image turn — keep answer in chat, no HTML artifact panel. */
...
export const buildPersistedAssistantPayload = (
  ...
  options?: PersistedAssistantOptions,
  ...
  if (options?.chatOnlyDocument) {

    const chatText = stripHtmlFencesForChat(cleanDisplayText(scanContent || stream.answer || raw));

    return {

      content: chatText || "No response generated.",
