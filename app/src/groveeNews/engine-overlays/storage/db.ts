...
export async function upsertRssItems(
  items: RssItem[],
  opts?: { fetchedAt?: number; live?: boolean },
): Promise<number> {
  ...
  await db.transaction("rw", db.rssItems, async () => {
    for (const item of allowed) {
      ...
      if (live && fetchedAt) {
        await db.rssItems.put({ ...item, fetchedAt });
        if (!exists) n++;
        continue;
      }
      if (!exists) {
        await db.rssItems.put({ ...item, fetchedAt: item.fetchedAt ?? 0 });
        n++;
      }
    }
  });
...
/** Headlines ingested during a live RSS scan session (online search). */
export async function getRssItemsSince(since: number, limit = 5000): Promise<RssItem[]> {
  const items = await db.rssItems.orderBy("publishedTs").reverse().limit(limit * 3).toArray();
  return items
    .filter((item) => !isBlockedRssItem(item) && (item.fetchedAt ?? 0) >= since)
    .slice(0, limit);
}
