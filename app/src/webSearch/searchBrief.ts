...
const scoreFact = (
  ...
  const newsKey = fact.match(/^\[חדשות \(([^)]+)\)\]/)?.[1];
  const feedKey = newsKey ? feedKeyFromSourceLabel(`חדשות (${newsKey})`) : null;
  if (feedKey && NEWS_FEEDS[feedKey]) {
    if (isWorldHeadlineQuery(query) && NEWS_FEEDS[feedKey].region !== "israel") score += 55;
    if (isWorldHeadlineQuery(query) && NEWS_FEEDS[feedKey].region === "israel") score -= 35;
    if (isIsraelNewsQuery(query) && NEWS_FEEDS[feedKey].region === "israel") score += 55;
    if (isIsraelNewsQuery(query) && NEWS_FEEDS[feedKey].region !== "israel") score -= 20;
  }
  return score;
};
...
export const formatSearchBriefContext = (
  ...
  const newsHeadlines = sources
    .filter((s) => s.provider === "news-rss" && s.ok && s.text.trim())
    .map((s) => {
      const m =
