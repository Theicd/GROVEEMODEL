...
const RC_EMBED = new URLSearchParams(location.search).get('embed') === 'grovee';
const _PROXY = '';  // Optional: Cloudflare Worker URL, e.g. 'https://my-proxy.workers.dev/?url='
let _serverBase = '';
let _standalone = false;
let _lastAviationFetch = 0;
const AVIATION_MIN_INTERVAL_MS = 55000;
const AVIATION_STATIC_INTERVAL_MS = 120000;
...
function _isViteDev() {
  return location.port === '5173' || location.port === '4173';
}

function _isStaticWebHost() {
  if (_isGitHubPages()) return true;
  if (_isViteDev()) return false;
  if (['localhost', '127.0.0.1'].includes(location.hostname) && location.port === '3000') return false;
  return true;
}

function _serverCandidates() {
  if (_isGitHubPages()) return [];
  if (_isViteDev()) return ['http://127.0.0.1:3000', 'http://localhost:3000'];
...
let _embedRegion = { countryCode: 'IL', label: 'ישראל', lat: 31.7683, lon: 35.2137, alt: 750000, flew: false };
let _alertReturnTimer = null;
let _presentationMode = false;
let _suppressUserRegionFly = false;
let _focusLabelEnt = null;
const ALERT_RETURN_MS = 5500;

const COUNTRY_REGIONS = {
  IL: { label: 'ישראל', lat: 31.7683, lon: 35.2137, alt: 750000 },
  FR: { label: 'צרפת', lat: 46.6, lon: 2.2, alt: 1100000 },
  DE: { label: 'גרמניה', lat: 51.1, lon: 10.4, alt: 1100000 },
  GB: { label: 'בריטania', lat: 54.0, lon: -2.5, alt: 1100000 },
  US: { label: 'ארה"ב', lat: 39.8, lon: -98.5, alt: 4500000 },
  TR: { label: 'טורקיה', lat: 39.0, lon: 35.2, alt: 1200000 },
  GR: { label: 'יוון', lat: 39.0, lon: 22.0, alt: 1000000 },
  EG: { label: 'מצרים', lat: 26.8, lon: 30.8, alt: 1500000 },
  JO: { label: 'ירדן', lat: 31.0, lon: 36.0, alt: 900000 },
  LB: { label: 'לבנון', lat: 33.9, lon: 35.5, alt: 800000 },
  ES: { label: 'ספרד', lat: 40.4, lon: -3.7, alt: 1200000 },
  IT: { label: 'איטליה', lat: 42.8, lon: 12.5, alt: 1100000 },
  JP: { label: 'יפן', lat: 36.2, lon: 138.2, alt: 1800000 },
  CN: { label: 'סין', lat: 35.8, lon: 104.1, alt: 3500000 },
  BR: { label: 'ברזיל', lat: -14.2, lon: -51.9, alt: 4500000 },
  CA: { label: 'קנדה', lat: 56.1, lon: -106.3, alt: 4000000 },
  MX: { label: 'מקסיקו', lat: 23.6, lon: -102.5, alt: 2500000 },
  RU: { label: 'רוסיה', lat: 61.5, lon: 105.3, alt: 5000000 },
  AU: { label: 'אוסטרליה', lat: -25.3, lon: 133.8, alt: 4000000 },
  PL: { label: 'פולין', lat: 51.9, lon: 19.1, alt: 1000000 },
  NL: { label: 'הולנד', lat: 52.1, lon: 5.3, alt: 900000 },
  BE: { label: 'בלגיה', lat: 50.5, lon: 4.5, alt: 900000 },
  SE: { label: 'שבדיה', lat: 60.1, lon: 18.6, alt: 1500000 },
  NO: { label: 'נורווגיה', lat: 60.5, lon: 8.5, alt: 1500000 },
  FI: { label: 'פינלנד', lat: 61.9, lon: 25.7, alt: 1500000 },
};

const PLACE_NAME_TO_CC = {
  'ישראל': 'IL', israel: 'IL',
  'גרמניה': 'DE', germany: 'DE', deutschland: 'DE',
  'צרפת': 'FR', france: 'FR',
  'בריטania': 'GB', 'בריטניה': 'GB', 'אנגליה': 'GB', uk: 'GB', 'united kingdom': 'GB', england: 'GB',
  'ארה"ב': 'US', 'ארצות הברית': 'US', usa: 'US', 'united states': 'US', america: 'US',
  'טורקיה': 'TR', turkey: 'TR',
  'יוון': 'GR', greece: 'GR',
  'מצרים': 'EG', egypt: 'EG',
  'ירדן': 'JO', jordan: 'JO',
  ...
  paris: 'FR', berlin: 'DE', london: 'GB', rome: 'IT', madrid: 'ES', tokyo: 'JP',
  moscow: 'RU', sydney: 'AU', 'tel aviv': 'IL', jerusalem: 'IL', haifa: 'IL',
};

function resolvePlaceGeo(name) {
  const raw = String(name || '').trim();
  if (!raw) return null;
  const lower = raw.toLowerCase();
  for (const [key, cc] of Object.entries(PLACE_NAME_TO_CC)) {
    if (lower === key.toLowerCase() || raw === key) {
      const p = COUNTRY_REGIONS[cc];
      if (p) return { lat: p.lat, lon: p.lon, alt: p.alt, label: p.label || raw, countryCode: cc };
    }
  }
  for (const [cc, p] of Object.entries(COUNTRY_REGIONS)) {
    if (p.label && (lower === p.label.toLowerCase() || raw === p.label)) {
      return { lat: p.lat, lon: p.lon, alt: p.alt, label: p.label, countryCode: cc };
    }
  }
  return null;
}

function setAllEmbedLayers(on) {
  for (const k of Object.keys(embedLayerVisible)) embedLayerVisible[k] = !!on;
  applyEmbedLayerVisibility();
  _trafficSig = _satSig = _worldAlertSig = _globalSig = '';
  refreshComposite();
  try {
    window.parent.postMessage({ source: 'reality-core', type: 'layers', payload: getEmbedLayers() }, '*');
  } catch (_) {}
}
...
function setPresentationMode(on) {
  ...
  if (_presentationMode) {
    if (_alertReturnTimer) { clearTimeout(_alertReturnTimer); _alertReturnTimer = null; }
    _suppressUserRegionFly = true;
    setAllEmbedLayers(false);
  }
  try {
    window.parent.postMessage({ source: 'reality-core', type: 'presentation', payload: { on: _presentationMode } }, '*');
  } catch (_) {}
}

function clearFocusLabel() {
  if (_focusLabelEnt && viewer) {
    try { viewer.entities.remove(_focusLabelEnt); } catch (_) {}
    _focusLabelEnt = null;
  }
}

function addFocusLabelAt(lon, lat, label) {
  if (!viewer || !label) return;
  clearFocusLabel();
  _focusLabelEnt = viewer.entities.add({
    position: Cesium.Cartesian3.fromDegrees(lon, lat, 0),
    label: {
      text: _rtl(String(label)),
...
function focusPlaceQuiet(lon, lat, alt = 900000, label = '') {
  if (!viewer || !Number.isFinite(lon) || !Number.isFinite(lat)) return;
  if (_alertReturnTimer) { clearTimeout(_alertReturnTimer); _alertReturnTimer = null; }
  _suppressUserRegionFly = true;
  _embedRegion.flew = true;
  setSceneMode('columbus', 0.35);
  viewer.camera.flyTo({
  ...
  flashEntityAt(lon, lat);
  if (label) addFocusLabelAt(lon, lat, label);
}

async function focusPlaceByName(name, alt) {
  const preset = resolvePlaceGeo(name);
  if (preset) {
    focusPlaceQuiet(preset.lon, preset.lat, alt || preset.alt || 900000, preset.label || name);
    return preset;
  }
  return null;
}

function getEmbedLayers() {
...
function toggleEmbedLayer(layer) {
  ...
  embedLayerVisible[layer] = !embedLayerVisible[layer];
  if (!wasOn && embedLayerVisible[layer]) {
    if (layer === 'aviation' || layer === 'ships') _trafficSig = '';
    if (layer === 'satellite') _satSig = '';
    if (layer === 'earthquake' || layer === 'weather') _worldAlertSig = '';
    refreshComposite();
  } else {
    applyEmbedLayerVisibility();
  }
...
function applyEmbedLayerVisibility() {
  if (!RC_EMBED) return;
  trafficMarks.forEach(e => {
    if (!e) return;
    const t = e._rcType;
    if (t === 'aviation') e.show = embedLayerVisible.aviation;
    if (t === 'ships') e.show = embedLayerVisible.ships;
  });
  satMarks.forEach(e => { if (e) e.show = embedLayerVisible.satellite; });
  globalMarks.forEach(e => {
    if (!e) return;
    const t = e._rcType;
    if (t === 'earthquake') e.show = embedLayerVisible.earthquake;
    if (t === 'weather') e.show = embedLayerVisible.weather;
  ...
  worldAlertMarks.forEach(e => {
    if (!e) return;
    const t = e._rcType;
    if (t === 'disaster-eq' || t === 'emsc-eq') e.show = embedLayerVisible.earthquake;
...
function setEmbedUserRegion(payload) {
  if (!RC_EMBED || !payload) return;
  const cc = String(payload.countryCode || 'IL').toUpperCase();
  const preset = COUNTRY_REGIONS[cc];
  _embedRegion = {
    countryCode: cc,
    label: payload.name || preset?.label || cc,
    lat: Number(payload.lat) || preset?.lat || 31.7683,
    lon: Number(payload.lon) || preset?.lon || 35.2137,
    alt: preset?.alt || 900000,
    flew: _embedRegion.flew,
  };
  if (!_embedRegion.flew && viewer && !_presentationMode && !_suppressUserRegionFly) {
    _embedRegion.flew = true;
    viewer.camera.flyTo({
      destination: Cesium.Cartesian3.fromDegrees(_embedRegion.lon, _embedRegion.lat, _embedRegion.alt),
      duration: 2.0,
      easingFunction: Cesium.EasingFunction.CUBIC_IN_OUT,
    });
  }
...
function buildEmbedHeadlines() {
  const headlines = [];
  const push = (id, text, severity) => headlines.push({ id, text, severity });
  if (currentIsraelAlert?.active) {
    push('il', `🇮🇱 צבע אדום: ${currentIsraelAlert.count || 0} אזורים`, 5);
  }
  const disasters = live.disasters?.items || [];
  disasters.sort((a, b) => gdacsSeverityScore(b.alertLevel) - gdacsSeverityScore(a.alertLevel)).slice(0, 2).forEach((d, i) => {
    push(`dis-${d.id || i}`, `🌍 ${cut(d.title || d.category || 'אירוע גלובלי', 55)}`, gdacsSeverityScore(d.alertLevel));
  });
  const galerts = live.global_alerts?.items || [];
  galerts.slice(0, 2).forEach((a, i) => {
    push(`nws-${a.id || i}`, `🌪 ${cut(a.headline || a.event || a.area || 'התרעת מזג אוויר', 55)}`, nwsSeverityScore(a.severity));
  });
  const eqAll = [...(live.earthquake?.items || []), ...(live.emsc_earthquakes?.items || []).map(e => ({
    magnitude: e.mag, place: e.place, geo: e.geo, time: e.time, id: e.id,
  }))];
  eqAll.sort((a, b) => (n(b.magnitude) || n(b.mag) || 0) - (n(a.magnitude) || n(a.mag) || 0)).slice(0, 2).forEach((eq, i) => {
    const mag = n(eq.magnitude) || n(eq.mag) || 0;
    if (mag >= 4) push(`eq-h-${eq.id || i}`, `⚠ M${mag.toFixed(1)} · ${cut(eq.place || '', 40)}`, mag >= 5 ? 5 : 4);
  });
  if (live.iss?.geo) {
    push('iss', `🛰 ISS · ${live.iss.geo.lat.toFixed(1)}°, ${live.iss.geo.lon.toFixed(1)}°`, 2);
  }
  const avN = live.aviation?.items?.length || 0;
  if (avN > 0) push('av', `✈ ${avN} מטוסים במעקב גלובלי`, 2);
  const regionLabel = _embedRegion?.label || 'עולם';
  if (!headlines.length) {
    push('hl-live', `REALITY LIVE · ${regionLabel} · מצב שגרה`, 2);
  }
  return headlines.slice(0, 8);
}

function buildEmbedGaugePayload() {
  const gauges = [];
  const eqAll = Array.isArray(live.earthquake?.items) ? live.earthquake.items : [];
  const emscAll = Array.isArray(live.emsc_earthquakes?.items) ? live.emsc_earthquakes.items : [];
  const mergedEq = [
    ...eqAll,
    ...emscAll.map(e => ({ magnitude: e.mag, place: e.place, depth: e.geo?.depth, geo: e.geo, time: e.time, id: e.id })),
  ];
  const eqNear = mergedEq.filter(x => x.geo && (inBounds(x.geo, REGION_BOUNDS) || inBounds(x.geo, RIFT_BOUNDS)))
    .sort((a, b) => (n(b.magnitude) || 0) - (n(a.magnitude) || 0));
  const topEq = eqNear[0] || [...mergedEq].sort((a, b) => (n(b.magnitude) || 0) - (n(a.magnitude) || 0))[0];
  if (topEq) {
    const mag = n(topEq.magnitude) || 0;
    const depth = n(topEq.depth);
    gauges.push({
      id: 'eq', label: 'EARTHQUAKE', icon: '⚠', value: `M ${mag.toFixed(1)}`,
      sub: `${cut(topEq.place || '', 28)}${depth !== null ? ` | ${Math.round(depth)} km` : ''}`,
      severity: mag >= 6 ? 5 : mag >= 4 ? 4 : 3,
    });
  }
  ...
  const topWave = [...marineNear].sort((a, b) => (n(b.waveHeight) || 0) - (n(a.waveHeight) || 0))[0]
    || [...marineAll].sort((a, b) => (n(b.waveHeight) || 0) - (n(a.waveHeight) || 0))[0];
  if (topWave) {
    const wh = n(topWave.waveHeight) || 0;
    gauges.push({
      id: 'wave', label: 'WAVES / SEA', icon: '🌊', value: `${wh.toFixed(1)} m`,
      sub: cut(topWave.station || '', 24),
      severity: wh >= 5 ? 4 : wh >= 3 ? 3 : 2,
    });
  }

  const wxAll = Array.isArray(live.weather?.items) ? live.weather.items : [];
  const wxPick = nearestWeatherTo(_embedRegion.lat, _embedRegion.lon, wxAll)
    || wxAll.find(x => x.geo && inBounds(x.geo, ISRAEL_BOUNDS)) || wxAll[0];
  if (wxPick) {
    const wind = n(wxPick.windspeed) || 0;
    const temp = n(wxPick.temperature);
    const parts = [cut(wxPick.city || _embedRegion.label || '', 16)];
    if (temp !== null) parts.push(`${Math.round(temp)}°C`);
    gauges.push({
      id: 'wind', label: 'WIND / TEMP', icon: '💨', value: `${Math.round(wind)} km/h`,
      sub: parts.join(' | '),
      severity: wind > 80 ? 4 : 2,
    });
  }
  ...
  if (sw) {
    ...
    gauges.push({
      id: 'space', label: 'SPACE WX', icon: '☄', value: `KP ${kp.toFixed(1)}`,
      sub: solarWind ? `רוח סולארית ${Math.round(solarWind)} km/s` : '',
      severity: kp >= 7 ? 5 : kp >= 5 ? 4 : 2,
    });
  }
  return gauges;
}

function insightToTicker(item, nowMs) {
  const cat = String(item.category || 'LIVE').replace(/^[⚠🌊🌡☄✈⛴🛰🚨📊🔗]\s*/, '');
  const icons = { SEISMIC: '⚠', TSUNAMI: '🌊', WEATHER: '🌡', MARITIME: '🌊', SPACE: '☄', AVIATION: '✈', SHIPS: '⛴', ISS: '🛰', SUMMARY: '📊', RED_ALERT: '🚨' };
  const ts = item.timestamp ? new Date(item.timestamp).getTime() : nowMs;
  const text = item.recommended_action && item.recommended_action !== 'מעקב שגרתי'
...
function showAlertFlashBrief() {
  ...
  if (RC_EMBED) {
    try { window.parent.postMessage({ source: 'reality-core', type: 'alert_flash' }, '*'); } catch (_) {}
  }
}

function focusEmbedHomeView(duration = 1.8) {
  if (!viewer) return;
  setSceneMode('columbus', 0);
  const r = _embedRegion;
  const homeIL = !r.countryCode || r.countryCode === 'IL';
  if (RC_EMBED && !homeIL) {
    viewer.camera.flyTo({
      destination: Cesium.Cartesian3.fromDegrees(r.lon, r.lat, r.alt || israelAlt),
      orientation: { heading: 0, pitch: -Cesium.Math.PI_OVER_TWO + 0.01, roll: 0 },
      duration,
      easingFunction: Cesium.EasingFunction.CUBIC_IN_OUT,
    });
  ...
  if (_mapMode === 'israel_flat' || _mapMode === 'auto' || RC_EMBED) {
    focusIsrael(duration);
    return;
  }
  if (_mapMode === 'flat') {
    viewer.camera.flyTo({
  ...
  viewer.camera.flyTo({
    destination: Cesium.Cartesian3.fromDegrees(r.lon, r.lat, r.alt || israelAlt),
    orientation: { heading: 0, pitch: -Cesium.Math.PI_OVER_TWO + 0.25, roll: 0 },
    duration,
    easingFunction: Cesium.EasingFunction.CUBIC_IN_OUT,
  });
}
...
function flyToAlertAndReturn(alert) {
  if (!viewer) return;
  const geo = alert?.geo || (Number.isFinite(alert?.lat) && Number.isFinite(alert?.lon)
    ? { lat: alert.lat, lon: alert.lon } : null);
  if (!geo || !Number.isFinite(geo.lat) || !Number.isFinite(geo.lon)) return;

  if (alert?.quiet || alert?.category === 'FOCUS') {
    focusPlaceQuiet(geo.lon, geo.lat, alert?.alt || 900000, alert?.label || '');
    return;
  }
  if (_presentationMode) return;
  const sev = Number(alert.severity || alert.sev || 2);
  const targetAlt = sev >= 5 ? 650000 : sev >= 4 ? 900000 : 1200000;

  if (RC_EMBED && _mapMode === 'globe') setSceneMode('columbus', 0.35);

  viewer.camera.flyTo({
    destination: Cesium.Cartesian3.fromDegrees(geo.lon, geo.lat, Math.min(targetAlt, israelAlt * 2)),
    orientation: (_mapMode === 'israel_flat' || _mapMode === 'flat' || RC_EMBED)
      ? { heading: 0, pitch: -Cesium.Math.PI_OVER_TWO + 0.01, roll: 0 }
      : undefined,
    duration: sev >= 4 ? 2.0 : 1.4,
    easingFunction: Cesium.EasingFunction.CUBIC_IN_OUT,
  });
  ...
  if (_alertReturnTimer) clearTimeout(_alertReturnTimer);
  _alertReturnTimer = setTimeout(() => focusEmbedHomeView(1.8), ALERT_RETURN_MS);
}

function postEmbedIntel() {
  if (!RC_EMBED || _presentationMode) return;
  const nowMs = Date.now();
  const clock = new Date(nowMs).toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
  const tickers = [];

  if (window.localAnalysis) {
    window.localAnalysis.run(live, { interval: 0 });
  }

  if (currentIsraelAlert?.active) {
    tickers.push({
      id: `il-alert-${currentIsraelAlert.id || nowMs}`,
      severity: 5, tag: 'צבע אדום', icon: '🚨', category: 'ISRAEL',
      text: `${currentIsraelAlert.count || 0} אזורים · ${cut(currentIsraelAlert.summary || '', 60)}`,
      time: clock, ts: nowMs,
      lat: ISRAEL_VIEW_3D.lat, lon: ISRAEL_VIEW_3D.lon,
    });
  }

  const avAll = Array.isArray(live.aviation?.items) ? live.aviation.items : [];
  avAll.filter(p => {
    if (!p.geo || !inBounds(p.geo, ISRAEL_EXT_BOUNDS)) return false;
    return p.isMilitary || _classifyAircraft(p.icao24, p.callsign, p.country, p.category).mil;
  }).slice(0, 5).forEach((p, i) => {
    const cls = p.milLabel || _classifyAircraft(p.icao24, p.callsign, p.country, p.category).label;
    tickers.push({
      id: `mil-${p.icao24 || i}`,
      severity: 5, tag: 'MILITARY', icon: '⚠', category: 'AVIATION',
      text: `${p.callsign || p.icao24 || '?'} · ${cls} · ${Math.round(p.altitude || 0)}m`,
      time: clock, ts: nowMs,
      lat: p.geo.lat, lon: p.geo.lon, geo: p.geo,
    });
  });

  const insights = window.localAnalysis?.getInsights?.() || [];
  insights.forEach(item => tickers.push(insightToTicker(item, nowMs)));

  buildHazardsRows().forEach((r, i) => {
    tickers.push({
      id: `hz-${i}-${r.t}-${Math.floor(nowMs / 30000)}`,
      severity: Number(r.s || 2), tag: r.t, category: 'HAZARD',
      text: r.d ? `${r.v} · ${r.d}` : r.v,
      time: clock, ts: nowMs,
      icon: /רעיד|RED|EARTH/i.test(r.t) ? '🌍' : /גל|ים|WAVE/i.test(r.t) ? '🌊' : /רוח|WIND|TEMP/i.test(r.t) ? '💨' : /חלל|SPACE/i.test(r.t) ? '☄' : '◆',
    });
  });
  ...
  const allEq = [
    ...eqAll,
    ...emscAll.map(e => ({ id: e.id, magnitude: e.mag, place: e.place, time: e.time, geo: e.geo })),
  ];
  allEq.sort((a, b) => (n(b.magnitude) || 0) - (n(a.magnitude) || 0)).slice(0, 10).forEach(eq => {
    const mag = n(eq.magnitude) || 0;
    const ts = eq.time ? new Date(eq.time).getTime() : nowMs;
    tickers.push({
      id: `eq-${eq.id || mag}`,
      severity: mag >= 5 ? 4 : mag >= 4 ? 3 : 2,
      tag: mag >= 5 ? 'BREAKING' : 'SEISMIC', icon: '🌍', category: 'SEISMIC',
      text: `M${mag.toFixed(1)} · ${cut(eq.place || '', 40)}`,
      time: new Date(ts).toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit' }),
      ts,
      lat: eq.geo?.lat, lon: eq.geo?.lon, geo: eq.geo || null,
    });
  });

  (live.disasters?.items || []).slice(0, 6).forEach((d, i) => {
    const sev = gdacsSeverityScore(d.alertLevel);
    const kind = String(d.category || '').toUpperCase();
    const icon = /TC|CYCL|TORN|HURR|STORM/i.test(kind) ? '🌪' : /FL|FLOOD/i.test(kind) ? '🌊' : /WF|FIRE/i.test(kind) ? '🔥' : /VO|VOL/i.test(kind) ? '🌋' : '⚠';
    tickers.push({
      id: `dis-${d.id || i}`,
      severity: sev, tag: kind || 'GLOBAL', icon, category: 'DISASTER',
      text: cut(d.title || d.category || 'אירוע גלובלי', 65),
      time: clock, ts: nowMs,
      lat: d.geo?.lat, lon: d.geo?.lon, geo: d.geo || null,
    });
  });

  (live.global_alerts?.items || []).slice(0, 6).forEach((a, i) => {
    tickers.push({
      id: `wxa-${a.id || i}`,
      severity: nwsSeverityScore(a.severity),
      tag: 'STORM', icon: '🌪', category: 'WEATHER',
      text: cut(a.headline || `${a.event || ''} · ${a.area || ''}`, 65),
      time: clock, ts: nowMs,
      lat: a.geo?.lat, lon: a.geo?.lon, geo: a.geo || null,
    });
  });

  if (live.iss?.geo) {
    tickers.push({
      id: 'iss-live', severity: 2, tag: 'ISS', icon: '🛰', category: 'ISS',
      text: `תחנת חלל · ${live.iss.geo.lat.toFixed(1)}°, ${live.iss.geo.lon.toFixed(1)}° · ${Math.round(live.iss.altitude || 408)} km`,
      time: clock, ts: nowMs,
      lat: live.iss.geo.lat, lon: live.iss.geo.lon, geo: live.iss.geo,
    });
  }

  const avCount = live.aviation?.items?.length || 0;
  if (avCount > 0) {
    tickers.push({
      id: `av-sum-${Math.floor(nowMs / 120000)}`, severity: 2, tag: 'AVIATION', icon: '✈', category: 'AVIATION',
      text: `${avCount} מטוסים במעקב גלובלי`,
      time: clock, ts: nowMs,
    });
  }

  const satCount = live.satellites?.items?.length || 0;
  if (satCount > 0) {
    tickers.push({
      id: `sat-sum-${Math.floor(nowMs / 120000)}`, severity: 2, tag: 'SAT', icon: '🛰', category: 'SPACE',
      text: `${satCount} לוויינים + ISS במעקב`,
      time: clock, ts: nowMs,
    });
  }
  ...
  if (!tickers.length) {
    tickers.push({
      id: 'idle', severity: 1, tag: 'LIVE', icon: '📡', category: 'LIVE',
      text: 'REALITY LIVE · טוען נתונים…', time: clock, ts: nowMs,
    });
  }

  const gauges = buildEmbedGaugePayload();
  const headlines = buildEmbedHeadlines();
  ...
  try {
    window.parent.postMessage({
      ...
      payload: {
        ...
        headlines,
        updatedAt: clock,
        layers: getEmbedLayers(),
        region: { countryCode: _embedRegion.countryCode, label: _embedRegion.label },
      },
    }, '*');
  } catch (_) {}
}

window.getEmbedLayers = getEmbedLayers;
window.toggleEmbedLayer = toggleEmbedLayer;
window.setEmbedUserRegion = setEmbedUserRegion;
window.flyToAlertAndReturn = flyToAlertAndReturn;
window.focusEmbedHomeView = focusEmbedHomeView;
window.setMapMode = setMapMode;
window.setPresentationMode = setPresentationMode;
window.setAllEmbedLayers = setAllEmbedLayers;
...
function extractGeoFromGeometry(geom) {
  ...
  if (geom.type === 'Polygon' && geom.coordinates[0]?.length) {
    ...
    let lat = 0; let lon = 0; let n = 0;
    for (const c of ring) {
      if (Number.isFinite(c[0]) && Number.isFinite(c[1])) { lon += c[0]; lat += c[1]; n++; }
    }
    return n ? { lat: lat / n, lon: lon / n } : null;
  }
  if (geom.type === 'MultiPolygon' && geom.coordinates[0]?.[0]?.length) {
    return extractGeoFromGeometry({ type: 'Polygon', coordinates: geom.coordinates[0] });
  }
  return null;
}

function nearestWeatherTo(lat, lon, items) {
  const list = Array.isArray(items) ? items.filter(w => w?.geo) : [];
  if (!list.length) return null;
  let best = list[0]; let bestD = Infinity;
  for (const w of list) {
    const d = Math.hypot((w.geo.lat - lat) * 111, (w.geo.lon - lon) * 85);
    if (d < bestD) { bestD = d; best = w; }
  }
  return best;
}

function nwsSeverityScore(sev) {
...
function gdacsSeverityScore(level) {
  const l = String(level || '').toLowerCase();
  if (l === 'red') return 5;
  if (l === 'orange') return 4;
  return 3;
}
...
function clearWorldAlertMarks() {
  worldAlertMarks = removeEntities(worldAlertMarks);
}
...
function enforceAutoMode() {
  if (!viewer || _mapMode !== 'auto' || RC_EMBED) return;
  let h = 0;
...
function initCesium() {
  ...
  let bootMode = RC_EMBED ? 'israel_flat' : 'israel_flat';
  if (!RC_EMBED) {
    try {
  ...
  setModeButtons();
  updateDepthReadout();
  setTimeout(() => setMapMode(bootMode, !RC_EMBED), 300);
  ...
  if (RC_EMBED && viewer) {
    ...
    ctrl.inertiaZoom = 0.72;
    ctrl.inertiaSpin = 0.85;
    ctrl.inertiaTranslate = 0.85;
    setTimeout(() => { try { viewer.resize(); viewer.scene.requestRender(); } catch(_) {} }, 100);
    setTimeout(() => { try { viewer.resize(); viewer.scene.requestRender(); } catch(_) {} }, 800);
  }
  ...
  viewer.camera.changed.addEventListener(() => {
    try {
      ...
      if (_mapMode === 'israel_flat' && !RC_EMBED) {
        setSceneMode('columbus', 0);
...
function renderTrafficLayer() {
  ...
  if (RC_EMBED && !embedLayerVisible.aviation && !embedLayerVisible.ships) {
    if (_trafficSig !== 'embed-off') { _trafficSig = 'embed-off'; clearTrafficMarks(); }
    return;
  }
...
function renderSatellitesLayer() {
  ...
  if (RC_EMBED && !embedLayerVisible.satellite) {
    if (_satSig !== 'embed-off') { _satSig = 'embed-off'; clearSatMarks(); }
...
function buildHazardsRows() {
  ...
  const disasters = live.disasters?.items || [];
  disasters.sort((a, b) => gdacsSeverityScore(b.alertLevel) - gdacsSeverityScore(a.alertLevel)).slice(0, 4).forEach(d => {
    const kind = String(d.category || d.title || '').slice(0, 24);
    rows.push({
      t: /TC|CYCL|TORN/i.test(kind) ? 'סופה / טורנדו' : /FL|FLOOD/i.test(kind) ? 'שיטפון' : /WF|FIRE/i.test(kind) ? 'שריפה' : /VO|VOL/i.test(kind) ? 'הר געש' : 'אירוע גלובלי',
      v: cut(d.title || kind, 42),
      s: gdacsSeverityScore(d.alertLevel),
      d: `${d.source || 'GDACS'}${d.alertLevel ? ` · ${d.alertLevel}` : ''}`,
    });
  });

  (live.global_alerts?.items || []).slice(0, 4).forEach(a => {
    rows.push({
      t: 'התרעת סופות',
      v: cut(a.event || a.headline || 'Weather alert', 42),
      s: nwsSeverityScore(a.severity),
      d: cut(a.area || a.source || '', 48),
    });
  });

  const emsc = live.emsc_earthquakes?.items || [];
  emsc.filter(e => (n(e.mag) || 0) >= 4).slice(0, 2).forEach(e => {
    rows.push({ t: 'רעידת אדמה', v: `M${(n(e.mag) || 0).toFixed(1)}`, s: (n(e.mag) || 0) >= 5 ? 4 : 3, d: cut(e.place || 'EMSC', 40) });
  });
...
function renderWorldAlertMarks() {
  if (!viewer) return;
  const global = isGlobalMode();
...
function checkGlobalHotAlerts() {
  ...
  mergedEq.filter(eq => eq.geo && (n(eq.magnitude) || 0) >= 5).forEach(eq => {
    ...
    pushAiAlert({
      ...
      severity: mag >= 6.5 ? 5 : mag >= 5.5 ? 4 : 4,
  ...
  (live.disasters?.items || []).filter(d => d.geo && gdacsSeverityScore(d.alertLevel) >= 4).forEach(d => {
    const key = `gdacs-${d.id || d.title}`;
    if (_seenAnomalies.has(key)) return;
    _seenAnomalies.add(key);
    const sev = gdacsSeverityScore(d.alertLevel);
    pushAiAlert({
  ...
  (live.global_alerts?.items || []).filter(a => a.geo && nwsSeverityScore(a.severity) >= 4).forEach(a => {
    ...
    pushAiAlert({
      ...
      category: 'WEATHER',
      severity: nwsSeverityScore(a.severity),
      summary: cut(a.headline || `${a.event || ''} · ${a.area || ''}`, 60),
      recommended_action: 'מעקב מזג אוויר',
      geo: a.geo,
    });
  });
}
...
async function _checkMilAlertCorrelation(milPlanes, now) {
  ...
  for (const p of milPlanes) {
    for (const area of _lastActiveAreas) {
      ...
      const cls = p.milLabel || _classifyAircraft(p.icao24, p.callsign, p.country, p.category).label;
      pushAiAlert({ id: key, category: 'CORRELATION', severity: 5, summary: `\u26a0\ufe0f \u05e1\u05d9\u05e0\u05db\u05e8\u05d5\u05df: ${cls} (${p.icao24}) ${Math.round(distKm)} \u05e7"\u05de \u05de\u05d0\u05d6\u05d5\u05e8 \u05d4\u05ea\u05e8\u05e2\u05d4 "${area}" | \u05de\u05e2\u05e7\u05d1 ${trackMin} \u05d3\u05e7\u05d5\u05ea | ${p.callsign || '---'} ${Math.round(p.altitude || 0)}m`, recommended_action: '\u05d7\u05e9\u05d3 \u05db\u05dc\u05d9 \u05d8\u05d9\u05e1 \u05d1\u05d0\u05d6\u05d5\u05e8 \u05d4\u05ea\u05e8\u05e2\u05d4 - \u05d0\u05e4\u05e9\u05e8\u05d9 \u05e7\u05e9\u05e8', geo: p.geo });
...
function refreshComposite() {
  ...
  updateDepthReadout();
  detectAnomalies();
  checkGlobalHotAlerts();
  if (window.localAnalysis && (_standalone || RC_EMBED)) {
    window.localAnalysis.run(live, RC_EMBED ? { interval: 0 } : undefined);
    if (_standalone) window.localAnalysis.feed(pushAiAlert);
  }
  renderAiPanel();
  if (RC_EMBED) postEmbedIntel();
}
...
function initWebSocket() {
  ...
  try {
    ...
    if (_serverBase) {
      try { wsHost = new URL(_serverBase).host; } catch (_) {}
    }
...
async function fetchPublicWeather() {
  ...
  try {
    const cities = [
      ...
      { name:'Eilat', lat:29.56, lon:34.95 }, { name:'Cairo', lat:30.04, lon:31.24 }, { name:'Amman', lat:31.95, lon:35.93 },
      { name:'Beirut', lat:33.89, lon:35.50 }, { name:'Istanbul', lat:41.01, lon:28.98 }, { name:'Riyadh', lat:24.69, lon:46.72 },
      { name:'Dubai', lat:25.20, lon:55.27 }, { name:'Tehran', lat:35.69, lon:51.39 }, { name:'Athens', lat:37.98, lon:23.73 },
      { name:'Ankara', lat:39.93, lon:32.86 }, { name:'Baghdad', lat:33.31, lon:44.37 }, { name:'Damascus', lat:33.51, lon:36.29 },
      { name:'Nairobi', lat:-1.29, lon:36.82 }, { name:'London', lat:51.51, lon:-0.13 }, { name:'Paris', lat:48.86, lon:2.35 },
      { name:'Berlin', lat:52.52, lon:13.41 }, { name:'Moscow', lat:55.76, lon:37.62 }, { name:'New York', lat:40.71, lon:-74.01 },
      { name:'Tokyo', lat:35.68, lon:139.69 }, { name:'Beijing', lat:39.90, lon:116.40 }, { name:'Sydney', lat:-33.87, lon:151.21 },
      { name:'Mumbai', lat:19.08, lon:72.88 }, { name:'Singapore', lat:1.35, lon:103.82 }, { name:'São Paulo', lat:-23.55, lon:-46.63 },
      { name:'Los Angeles', lat:34.05, lon:-118.24 }, { name:'Chicago', lat:41.88, lon:-87.63 }, { name:'Johannesburg', lat:-26.20, lon:28.04 }
    ];
...
async function fetchPublicSatellites() {
  const t0 = performance.now();
  const jsonUrl = 'https://celestrak.org/NORAD/elements/gp.php?GROUP=active&FORMAT=json';
  _log('Satellites', `→ ${jsonUrl}`);
  try {
    const arr = await _fetchRemote(jsonUrl, { timeout: 15000, json: true });
    if (Array.isArray(arr) && arr.length) {
      const items = arr.slice(0, 200).map(s => ({
        name: s.OBJECT_NAME, noradId: s.NORAD_CAT_ID,
        tle1: s.TLE_LINE1 || '', tle2: s.TLE_LINE2 || '',
        inclination: s.INCLINATION, meanMotion: s.MEAN_MOTION,
      })).filter(s => s.tle1 && s.tle2);
      if (items.length) {
        live.satellites = { timestamp: new Date().toISOString(), items };
        _log('Satellites', `✓ ${items.length} satellites via JSON (${(performance.now()-t0).toFixed(0)}ms)`);
        return;
      }
    }
  } catch (e) { _logWarn('Satellites', `✗ JSON: ${e?.message||e}`); }

  const tleUrl = 'https://celestrak.org/NORAD/elements/gp.php?GROUP=visual&FORMAT=tle';
  const text = await _fetchRemote(tleUrl, { timeout: 15000, json: false });
    ...
    try {
      ...
      for (let i = 0; i + 2 < lines.length; i += 3) {
        ...
        if (tle1.startsWith('1 ') && tle2.startsWith('2 ')) {
          items.push({ name, noradId: parseInt(tle1.substring(2, 7)), tle1, tle2 });
        }
        if (items.length >= 200) break;
      }
      if (items.length) {
        live.satellites = { timestamp: new Date().toISOString(), items };
        _log('Satellites', `✓ ${items.length} satellites via TLE (${(performance.now()-t0).toFixed(0)}ms)`);
        return;
      }
    } catch (e) { _logWarn('Satellites', `✗ TLE parse: ${e?.message||e}`); }
  }

  const items = buildFallbackSatellites();
  live.satellites = { timestamp: new Date().toISOString(), items };
  _logWarn('Satellites', `→ Using ${items.length} fallback satellites (${(performance.now()-t0).toFixed(0)}ms)`);
}
...
async function fetchPublicMarine() {
  ...
  try {
    const pts = [
      ...
      {lat:51.0,lon:1.5,n:'Channel'},{lat:22.3,lon:114.2,n:'SouthChina'},{lat:-22.9,lon:-43.2,n:'Brazil-Coast'},
      {lat:37.8,lon:-122.4,n:'US-Pacific'},{lat:43.7,lon:-70.2,n:'US-Atlantic'}
    ];
...
async function fetchPublicISS() {
  const t0 = performance.now();
  const sources = [
    { url: 'https://api.wheretheiss.at/v1/satellites/25544', parse: d => d?.latitude != null ? { lat: d.latitude, lon: d.longitude, alt: d.altitude } : null },
    { url: 'http://api.open-notify.org/iss-now.json', parse: d => d?.iss_position ? { lat: parseFloat(d.iss_position.latitude), lon: parseFloat(d.iss_position.longitude), alt: 408 } : null },
  ];
  for (const src of sources) {
    _log('ISS', `→ ${src.url}`);
    try {
      const d = await _fetchRemote(src.url, { timeout: 10000, json: true });
      const pos = src.parse(d);
      if (pos && Number.isFinite(pos.lat) && Number.isFinite(pos.lon)) {
        live.iss = { geo: { lat: pos.lat, lon: pos.lon }, altitude: pos.alt || 408 };
        _log('ISS', `✓ lat=${pos.lat.toFixed(1)} lon=${pos.lon.toFixed(1)} (${(performance.now()-t0).toFixed(0)}ms)`);
        return;
      }
    } catch (e) { _logWarn('ISS', `✗ ${src.url}: ${e?.message||e}`); }
  }
  _logErr('ISS', `✗ All ISS sources failed (${(performance.now()-t0).toFixed(0)}ms)`);
}
...
function _mapOpenSkyStates(states) {
  if (!states?.length) return [];
  return states.filter(s => s[5] != null && s[6] != null && !s[8]).map(s => {
...
function _mergeAviationByIcao(lists) {
  const m = new Map();
  for (const list of lists) {
    for (const item of list || []) {
      const id = item?.icao24 || `${item?.geo?.lat},${item?.geo?.lon}`;
      if (id) m.set(id, item);
    }
  }
  return [...m.values()];
...
const AVIATION_GLOBAL_HUBS = [
  { lat: 31.5, lon: 34.8 }, { lat: 40.7, lon: -74.0 }, { lat: 51.47, lon: -0.46 },
  { lat: 35.68, lon: 139.69 }, { lat: 25.25, lon: 55.36 },
];

async function _fetchAviationOpenSkyRegional() {
  ...
  _log('Aviation', '→ OpenSky regional (proxied)...');
  const d = await _fetchRemote(url, { timeout: 12000, json: true });
  if (!d) throw new Error('no data');
  return _mapOpenSkyStates(d?.states);
}

async function _fetchAviationOpenSkyGlobal() {
  const url = 'https://opensky-network.org/api/states/all';
  _log('Aviation', '→ OpenSky GLOBAL (proxied, like server)...');
  const d = await _fetchRemote(url, { timeout: 15000, json: true });
  if (!d) throw new Error('no data');
  return _mapOpenSkyStates(d?.states).slice(0, 900);
}

async function _fetchAviationADSBLiveRegional() {
  ...
  _log('Aviation', `→ airplanes.live regional (proxied): ${url}`);
  const d = await _fetchRemote(url, { timeout: 10000, json: true });
  if (!d) throw new Error('no data');
  const ac = d?.ac || [];
  if (!ac.length) return [];
  return ac.filter(a => Number.isFinite(a.lat) && Number.isFinite(a.lon)).map(_mapADSBLiveAc);
}
...
function buildFallbackFlights() {
  const hubs = [
    ...
    { n: 'LY315', c: 'IL', lat: 32.01, lon: 34.89, h: 90 },
  ];
  return hubs.map((h, i) => ({
    icao24: `sim${i}`, callsign: h.n, country: h.c,
    geo: { lat: h.lat, lon: h.lon }, altitude: 9000 + (i % 5) * 1500,
    velocity: 220 + (i % 7) * 18, heading: h.h, onGround: false,
    timestamp: Date.now(), source: 'fallback',
  }));
}

function buildFallbackSatellites() {
  return [
    { name: 'ISS (ZARYA)', noradId: 25544, tle1: '1 25544U 98067A   24001.50000000  .00010000  00000+0  10000-4 0  9993', tle2: '2 25544  51.6400 100.0000 0001001  90.0000 270.0000 15.50000000200000' },
    { name: 'HST', noradId: 20580, tle1: '1 20580U 90037B   24001.50000000  .00010000  00000+0  10000-4 0  9990', tle2: '2 20580  28.4700 120.0000 0002001  80.0000 280.0000 15.10000000150000' },
    { name: 'NOAA 19', noradId: 33591, tle1: '1 33591U 09005A   24001.50000000  .00010000  00000+0  10000-4 0  9991', tle2: '2 33591  99.2000 200.0000 0013001  70.0000 290.0000 14.20000000140000' },
    { name: 'TIANGONG', noradId: 48274, tle1: '1 48274U 21035A   24001.50000000  .00010000  00000+0  10000-4 0  9995', tle2: '2 48274  41.5000  50.0000 0004001  60.0000 300.0000 15.60000000160000' },
  ];
}
...
async function fetchPublicAviation() {
  const t0 = performance.now();
  const now = Date.now();
  const minIv = _isStaticWebHost() ? AVIATION_STATIC_INTERVAL_MS : AVIATION_MIN_INTERVAL_MS;
  if (now - _lastAviationFetch < minIv && live.aviation?.items?.length) {
    return;
  }
  const wantGlobal = wantsGlobalDataFeed();
  _log('Aviation', `→ Fetching aircraft (${wantGlobal ? 'global' : 'regional'}, static=${_isStaticWebHost()})...`);
  const chunks = [];

  try {
    const os = (_isStaticWebHost() && wantGlobal)
      ? await _fetchAviationOpenSkyRegional()
      : (wantGlobal ? await _fetchAviationOpenSkyGlobal() : await _fetchAviationOpenSkyRegional());
  ...
  if (!_mergeAviationByIcao(chunks).length && !_isStaticWebHost()) {
    ...
    } catch (e) { _logWarn('Aviation', `✗ airplanes.live: ${e?.message||e}`); }
  }

  let items = _mergeAviationByIcao(chunks);
  if (wantGlobal && items.length > 900) items = items.slice(0, 900);

  if (!items.length) {
    items = buildFallbackFlights();
    _logWarn('Aviation', `→ Using ${items.length} fallback aircraft (API unavailable)`);
  }

  _lastAviationFetch = now;
  _trafficSig = '';
  const mil = items.filter(i => i.isMilitary).length;
  live.aviation = { timestamp: new Date().toISOString(), items };
  _log('Aviation', `✓ ${items.length} aircraft (${mil} military, ${items[0]?.source === 'fallback' ? 'fallback' : wantGlobal ? 'global' : 'regional'}) in ${(performance.now()-t0).toFixed(0)}ms`);
}
...
async function fetchPublicShips() {
  ...
  const worldPorts = wantsGlobalDataFeed() ? [
    ...
    {name:'Panama Transit',lat:9.08,lon:-79.68,h:90,t:80,dst:'PAPTY'},{name:'Cape Town',lat:-33.92,lon:18.42,h:180,t:70,dst:'ZACPT'},
    {name:'Sydney',lat:-33.86,lon:151.20,h:135,t:60,dst:'AUSYD'},{name:'Mumbai',lat:18.94,lon:72.85,h:270,t:70,dst:'INBOM'},
    {name:'Hamburg',lat:53.55,lon:9.99,h:45,t:70,dst:'DEHAM'},{name:'Busan',lat:35.10,lon:129.04,h:180,t:70,dst:'KRPUS'},
    {name:'Santos',lat:-23.95,lon:-46.33,h:90,t:70,dst:'BRSSZ'},{name:'Vancouver',lat:49.28,lon:-123.12,h:315,t:70,dst:'CAVAN'},
  ...
  if (items.length) {
    ...
    _log('Ships', `✓ Total: ${items.length} ships (${realItems.length} real + ${medItems.length} route markers) in ${(performance.now()-t0).toFixed(0)}ms`);
  } else {
...
async function fetchPublicFires() {
  ...
  try {
    const text = await _fetchRemote(url, { timeout: 15000, json: false });
...
async function fetchPublicGlobalAlerts() {
  const t0 = performance.now();
  _log('GlobalAlerts', '→ Fetching NWS + global weather alerts...');
  const items = [];
  ...
  try {
    ...
    if (d) {
      ...
      _log('GlobalAlerts', `✓ NWS: ${d.features?.length||0} alerts (${items.filter(x => x.geo).length} with geo)`);
    }
...
function _localDevProxyUrl(targetUrl) {
  if (!_isViteDev()) return '';
...
async function _fetchRemote(url, { timeout = 10000, json = true, headers = {} } = {}) {
  const blocked = _hostNeedsProxy(url);
  const init = { signal: AbortSignal.timeout(timeout), headers: { Accept: 'application/json, text/plain, */*', ...headers } };
  ...
  for (const step of chain) {
    try {
      ...
      if (r.status === 429) {
        _logWarn('Fetch', `${step.name} rate limited (429) for ${url.substring(0, 50)}`);
        return null;
      }
      if (!r.ok) continue;
      if (json) {
      ...
      if (!_isStaticWebHost()) _logWarn('Fetch', `${step.name}: ${e?.message || e}`);
    }
  }
  return null;
}

async function tryConnectRealityServer() {
  const candidates = _serverCandidates();
  ...
  for (const base of candidates) {
    ...
    seen.add(base);
    try {
      _log('Init', `→ Trying ${base}/api/data ...`);
      const r = await fetch(`${base}/api/data`, { signal: AbortSignal.timeout(4500) });
      if (!r.ok) continue;
...
async function serverDataPoll() {
  if (_standalone || !_serverBase) return;
  try {
    const r = await fetch(`${_serverBase}/api/data`, { signal: AbortSignal.timeout(8000) });
    if (r.ok) {
      const all = await r.json();
      if (all && !all.error) { absorbAllData(all); refreshComposite(); }
    }
  } catch (_) {}
}

async function _corsFetch(url, timeout = 10000) {
  const text = await _fetchRemote(url, { timeout, json: false });
  return text || null;
}
...
async function prime() {
  ...
  const isEmbed = RC_EMBED || new URLSearchParams(location.search).get('standalone') === '1';
  _log('Init', `═══ prime() START — host=${location.hostname} embed=${isEmbed} isGH=${isGH} static=${_isStaticWebHost()} ═══`);

  if (!_isStaticWebHost()) {
    const all = await tryConnectRealityServer();
    if (all) {
      _standalone = false;
      const t0 = performance.now();
      _log('Init', `✓ Reality Core server at ${_serverBase} (${(performance.now()-t0).toFixed(0)}ms) → SERVER MODE`);
      absorbAllData(all);
      try {
        const r = await fetch(`${_serverBase}/api/alerts?min_severity=2`, { signal: AbortSignal.timeout(5000) });
        if (r.ok) {
          const alerts = await r.json();
          if (Array.isArray(alerts)) { aiAlerts = alerts.slice(0, 40); _log('Init', `✓ ${alerts.length} alerts loaded`); }
        }
      } catch (_) {}
      setConn(true);
      const connEl = $('conn');
      if (connEl) connEl.innerHTML = `LIVE · SERVER <span id="connDot" style="display:inline-block;width:8px;height:8px;border-radius:50%;margin-left:6px;background:#00ff88;box-shadow:0 0 10px rgba(0,255,136,.35)"></span>`;
      refreshComposite();
      return;
    }
  }
  ...
  _standalone = true;
  _log('Init', `═══ STANDALONE MODE (${isEmbed ? 'GROVEE' : isGH ? 'GitHub Pages' : isLocal ? 'local' : location.hostname}) ═══`);
  setConn(false);
  const connEl = $('conn');
  if (connEl) connEl.innerHTML = `LIVE${isEmbed ? ' · WEB' : isGH ? ' · GH' : ''} <span id="connDot" style="display:inline-block;width:8px;height:8px;border-radius:50%;margin-left:6px;background:#00ff88;box-shadow:0 0 10px rgba(0,255,136,.35)"></span>`;
  const aiLabel = document.querySelector('#aiPanel b');
  if (aiLabel) aiLabel.textContent = '📊 LOCAL MONITOR';
  await standaloneRefresh();
}

async function bootIsraelApp() {
  if (RC_EMBED) {
    document.documentElement.classList.add('embed-grovee-root');
    document.body.classList.add('embed-grovee');
  }
  console.log('%c███ REALITY-CORE v39 — Starting... ███', 'color:#00e5ff;font-size:16px;font-weight:bold;text-shadow:0 0 8px #00e5ff');
  ...
  if (!_standalone) {
    initWebSocket();
    setInterval(serverDataPoll, 8000);
  }
  setTimeout(() => {
    if (typeof QA !== 'undefined' && typeof QA.runQuick === 'function') {
      _log('QA', '→ Running auto quick check...');
      const r = QA.runQuick();
      _log('QA', `✓ ${r.pass}/${r.total} passed (${r.percentage}%)`);
    }
  }, 15000);
  ...
  $('aiHistory')?.addEventListener('click', (e) => {
    ...
    if (geo) {
      flyToAlertAndReturn({ geo, severity: Number(target.severity || target.sev || 4), category: target.category || 'ALERT' });
    }
  });
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => { void bootIsraelApp(); });
} else {
  void bootIsraelApp();
}
